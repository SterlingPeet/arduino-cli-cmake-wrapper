...
def main(arguments: List[str] = None):
    ...
    try:
        ...
        # Convert --properties to --build-property args for arduino-cli
        build_args = []
        if arguments.properties:
            for prop in arguments.properties:
                build_args.extend(["--build-property", prop])

        # Run the build
        test_file_map, stdout, stderr = build(arguments.board, arguments.libraries, build_args)
