...
def invocation_filter(lines: List[str]) -> List[str]:
    ...
    return [
        line
        for line, shell_tokens in [
            (line, safe_split(line, ignore_errors=True)) for line in lines
        ]
...
def parse(stdout: str) -> Dict[Stage, List[str]]:
    ...
    unknown_titles = [
        title
        for title, stage in zip(
            [title for title, _ in sections], [stage for stage, _ in annotated]
        )
        if stage == Stage.UNKNOWN
    ]
    if unknown_titles:
        LOGGER.warning(
            'Unknown sections detected with titles: %s',
            ','.join(unknown_titles),
        )
